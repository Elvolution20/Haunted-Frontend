...
contract Haunted is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the HAUNTED ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HAUNTED", "HAUNTED") {
        // Mints 1 HAUNTED to contract creator for initial pool setup
    ...
    function _getHauntedPrice() internal view returns (uint256 _hauntedPrice) {
        try IOracle(hauntedOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Haunted: failed to fetch HAUNTED price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _hauntedPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_hauntedPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHauntedOracle(address _hauntedOracle) public onlyOperatorOrTaxOffice {
        require(_hauntedOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints HAUNTED to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HAUNTED to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHauntedPrice = _getHauntedPrice();
            currentTaxRate = _updateTaxRate(currentHauntedPrice);
            if (currentHauntedPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _hauntedPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_hauntedPool != address(0), "!_hauntedPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_hauntedPool, INITIAL_HAUNTED_POOL_DISTRIBUTION);
