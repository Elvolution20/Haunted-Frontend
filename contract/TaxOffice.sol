...
interface ITaxable {
    ...
    function setHauntedOracle(address _hauntedOracle) external;
...
contract TaxOffice is Operator {
    ...
    function setTaxTiersTwap(uint8 _index, uint256 _value) public onlyOperator returns (bool) {
        return ITaxable(haunted).setTaxTiersTwap(_index, _value);
    }

    function setTaxTiersRate(uint8 _index, uint256 _value) public onlyOperator returns (bool) {
        return ITaxable(haunted).setTaxTiersRate(_index, _value);
    }

    function enableAutoCalculateTax() public onlyOperator {
        ITaxable(haunted).enableAutoCalculateTax();
    }

    function disableAutoCalculateTax() public onlyOperator {
        ITaxable(haunted).disableAutoCalculateTax();
    }

    function setTaxRate(uint256 _taxRate) public onlyOperator {
        ITaxable(haunted).setTaxRate(_taxRate);
    }

    function setBurnThreshold(uint256 _burnThreshold) public onlyOperator {
        ITaxable(haunted).setBurnThreshold(_burnThreshold);
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) public onlyOperator {
        ITaxable(haunted).setTaxCollectorAddress(_taxCollectorAddress);
    }
    ...
    function _excludeAddressFromTax(address _address) private returns (bool) {
        if (!ITaxable(haunted).isAddressExcluded(_address)) {
            return ITaxable(haunted).excludeAddress(_address);
        }
    }
    ...
    function _includeAddressInTax(address _address) private returns (bool) {
        if (ITaxable(haunted).isAddressExcluded(_address)) {
            return ITaxable(haunted).includeAddress(_address);
        }
    }

    function taxRate() external view returns (uint256) {
        return ITaxable(haunted).taxRate();
    }
    ...
    {
        require(amtHaunted != 0 && amtToken != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);

        IERC20(haunted).transferFrom(msg.sender, address(this), amtHaunted);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(haunted, uniRouter);
        ...
        uint256 liquidity;
        (resultAmtHaunted, resultAmtToken, liquidity) = IUniswapV2Router(uniRouter).addLiquidity(
            haunted,
            token,
            amtHaunted,
            amtToken,
        ...
        return (resultAmtHaunted, resultAmtToken, liquidity);
    }
    ...
    {
        require(amtHaunted != 0 && msg.value != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);
    ...
    function setTaxableHauntedOracle(address _hauntedOracle) external onlyOperator {
        ITaxable(haunted).setHauntedOracle(_hauntedOracle);
    }

    function transferTaxOffice(address _newTaxOffice) external onlyOperator {
        ITaxable(haunted).setTaxOffice(_newTaxOffice);
    }
    ...
    function taxFreeTransferFrom(
        ...
        IERC20(haunted).transferFrom(_sender, _recipient, _amt);
