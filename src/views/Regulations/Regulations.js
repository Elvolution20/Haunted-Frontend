...
const Regulations = () => {
  ...
  function createData(epoch, dao, dev, staking, bondsBought, bondsRedeemed) {
    var sum = (Number(dao) + Number(dev) + Number(staking)).toFixed(2);
    return { epoch, dao, dev, staking, sum, bondsBought, bondsRedeemed };
  }
  useEffect(() => {
    if (hauntedFinance) {
      const thisData = hauntedFinance.listenForRegulationsEvents();
        ...
        <Table className={classes.table} aria-label="simple table">
          <TableHead>
            <TableRow>
              ...
              <StyledTableCell align="center">Staking funding</StyledTableCell>
          ...
          <TableBody>
            {rows?.map((row, index) => (
              ...
              >
                ...
                <StyledTableCell align="center">{row.staking}</StyledTableCell>
